...
public class RagdollModel extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		if(bodies.size <= PART_RIGHT_SHIN) {
			return false;
		}

		Fixture bodyFix = bodies.get(PART_BODY).getBody().getFixtureList().get(0);

		Vector2 aCache = new Vector2();
		Vector2 bCache = new Vector2();
...
//		private static final float SHIN_OFFSET    = 2.75f;
		...
		addJoint(PART_BODY, PART_HEAD,
				aCache.set(0.f, TORSO_OFFSET / 2.f), bCache.set(0.f, -TORSO_OFFSET / 2.f), world);

		addJoint(PART_BODY, PART_LEFT_ARM,
				aCache.set(-ARM_XOFFSET / 2.f, ARM_YOFFSET), bCache.set(ARM_XOFFSET / 2.f, 0.f), world);

		addJoint(PART_LEFT_ARM, PART_LEFT_FOREARM,
				aCache.set(-FOREARM_OFFSET / 2.f, 0.f), bCache.set(FOREARM_OFFSET / 2.f, 0.f), world);

		addJoint(PART_BODY, PART_RIGHT_ARM,
				aCache.set(ARM_XOFFSET / 2.f, ARM_YOFFSET), bCache.set(ARM_XOFFSET / 2.f, 0.f), world);

		addJoint(PART_RIGHT_ARM, PART_RIGHT_FOREARM,
				aCache.set(-FOREARM_OFFSET / 2.f, 0.f), bCache.set(FOREARM_OFFSET / 2.f, 0.f), world);

		addJoint(PART_BODY, PART_LEFT_THIGH,
				aCache.set(-THIGH_XOFFSET, -THIGH_YOFFSET / 2.f), bCache.set(0.f, THIGH_YOFFSET / 2.f), world);

		addJoint(PART_LEFT_THIGH, PART_LEFT_SHIN,
				aCache.set(0.f, -SHIN_OFFSET / 2.f), bCache.set(0.f, SHIN_OFFSET / 2.f), world);

		addJoint(PART_BODY, PART_RIGHT_THIGH,
				aCache.set(THIGH_XOFFSET, -THIGH_YOFFSET / 2.f), bCache.set(0.f, THIGH_YOFFSET / 2.f), world);

		addJoint(PART_RIGHT_THIGH, PART_RIGHT_SHIN,
				aCache.set(0.f, -SHIN_OFFSET / 2.f), bCache.set(0.f, SHIN_OFFSET / 2.f), world);
	...
	/**
	 * Adds joints between the specified body parts.
	 *
	 * @param indexA The index in bodies of the first segment of the joint
	 * @param indexB The index in bodies of the second segment of the joint
	 * @param offsetA A Vector2 with the offset from the center of body A, where the joint should be attached
	 * @param offsetB A Vector2 with the offset from the center of body B, where the joint should be attached
	 * @param world Box2D world to store joints
	 *
	 * Requires: indexA and indexB must be valid indices of the bodies array
	 */
	protected boolean addJoint(int indexA, int indexB, Vector2 offsetA, Vector2 offsetB, World world) {
		// Definition for a revolute joint
		RevoluteJointDef jointDef = new RevoluteJointDef();

		// Initial joint
		jointDef.bodyA = bodies.get(indexA).getBody();
		jointDef.bodyB = bodies.get(indexB).getBody();
		jointDef.localAnchorA.set(offsetA);
		jointDef.localAnchorB.set(offsetB);
